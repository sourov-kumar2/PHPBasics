...
?>
 <?php 
    ...
    if(isset($_POST['changePass'])){
        $oldpass = $conn-> real_escape_string(safeData($_POST['oldPassword']));
        $newpass = $conn-> real_escape_string(safeData($_POST['newPassword']));
        ...
        if($result -> num_rows > 0){
            ...
            if(password_verify($oldpass,$row['password'])){
                ...
                $sql = "UPDATE users  SET   `password` = '$newpass' WHERE `id` =" .$_SESSION['user']['id'];
                if($conn-> query($sql)){
